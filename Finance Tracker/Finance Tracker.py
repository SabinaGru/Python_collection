"""
Track and log different Transactions
Organize Them
...
class CSV:
    ...
    def get_transactions(cls, start_date, end_date):
        ...
        # Convert Datetimes n start/enddate
        df["date"] = pd.to_datetime(df["date"], format = CSV.FORMAT)
        start_date = datetime.strptime(start_date, CSV.FORMAT)
        end_date = datetime.strptime(end_date, CSV.FORMAT)

        # Mask for filtering rows in Df
        mask = (df["date"] >= start_date) & (df["date"] <= end_date)
        filtered_df = df.loc[mask]

        if filtered_df.empty:
            print('No transactions found in the given date range')
            return None  # Return None if no transactions found
        else:
            print(
            ...
            print("\nSummary:")
            print(f"Total Income: €{total_income:.2f}")     # .2f -> round to 2 Decimal
...
# empty days must be marked as 0
def plot_transactions(df):
    df.set_index("date", inplace=True)

    income_df = (df[df["category"] == "Income"]
                 .resample("D")                       # filling in rows for every single day & aggregation; "D" -> Daily Frequency
                 .sum()                               # sum for value for same day
                 .reindex(df.index, fill_value=0)   # correct re-index-ing and filling missing values
                 )
